...
class OverlayService : Service() {
    ...
    private fun createNotification(): Notification {
        val channelId = "freeride_overlay"
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(channelId, "FreeRide Overlay", NotificationManager.IMPORTANCE_LOW)
        ...
        return Notification.Builder(this, channelId)
            .setContentTitle("FreeRide Active")
    ...
    private fun startRide() {
        ...
        // Bike sensor via WorkoutSessionClient (VeloLauncher AIDL session)
        val client = WorkoutSessionClient(this)
        bikeClient = client
        client.addListener(object : WorkoutSessionClient.ListenerAdapter() {
            override fun onSensorData(resistance_: Int, rpm_: Int, tilt: Int, power_: Float,
                ...
                speedMph = (speedMps * 2.24).toFloat().coerceAtMost(45f)
                ...
                calories = ((avgPower / 0.25) * elapsedHours / 1.163).toInt()
            ...
            override fun onConnectionChanged(isConnected: Boolean, message: String?) {
    ...
    private fun stopRide() {
        ...
        // Show summary screen if ride was >= 1 minute
        if (elapsed >= 60) {
    ...
    companion object {
        const val ACTION_START = "io.freewheel.freeride.OVERLAY_START"
        const val ACTION_STOP = "io.freewheel.freeride.OVERLAY_STOP"
        const val ACTION_TOGGLE = "io.freewheel.freeride.OVERLAY_TOGGLE"
