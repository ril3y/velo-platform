#!/bin/bash
# Platform-sign the FreewheelBridge APK for Bowflex device (uid=1000 system access)
#
# Key sources (in priority order):
#   1. Environment vars: PLATFORM_PK8_PATH + PLATFORM_CERT_PATH
#   2. Local files: freewheelbridge/signing/platform.pk8 + device_platform.x509.pem
#
...
APK_IN="$SCRIPT_DIR/build/outputs/apk/debug/freewheelbridge-debug.apk"
APK_ALIGNED="$SCRIPT_DIR/build/outputs/apk/freewheelbridge-aligned.apk"
APK_OUT="$SCRIPT_DIR/build/outputs/apk/freewheelbridge.apk"
...
if [ ! -f "$PLATFORM_PK8" ]; then
    ...
    echo "Either place keys in freewheelbridge/signing/ or set PLATFORM_PK8_PATH env var"
    exit 1
fi
if [ ! -f "$PLATFORM_CERT" ]; then
    echo "ERROR: Platform cert not found at $PLATFORM_CERT"
    echo "Either place keys in freewheelbridge/signing/ or set PLATFORM_CERT_PATH env var"
    exit 1
fi

if [ ! -f "$APK_IN" ]; then
    echo "ERROR: Debug APK not found at $APK_IN"
    echo "Run './gradlew :freewheelbridge:assembleDebug' first."
    exit 1
fi
